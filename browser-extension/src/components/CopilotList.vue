  ...
  <PageOverlay>
    ...
    <template v-slot:content>
      <div class="flex flex-row py-3">
        <div class="flex items-center text-base cursor-pointer" @click="showAddCopilot = true">
          <circle-plus-icon />{{ t("addTitle") }}
        </div>
      </div>
      <div v-for="(agent, index) in agents" :key="agent.manifest.id" class="flex flex-row py-3">
        <div
          class="flex flex-row flex-auto self-center items-center cursor-pointer"
          @click="$emit('activateAgent', agent.manifest.id)"
        >
    ...
    <template v-slot:modalsContainer>
      <ModalForm
        :title="t('removeTitle')"
        :show="deletingIndex >= 0"
        @close="closeDeletionConfirmation"
        @save="confirmRemoval"
        :button-text="t('removeButton')"
      >
        {{
