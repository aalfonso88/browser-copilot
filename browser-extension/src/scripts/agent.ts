...
export class Agent {
  ...
  public async *ask(msg: string, sessionId: string, authService?: AuthService): AsyncIterable<string | AgentFlow> {
    const ret = await fetchStreamJson(
      ...
      await this.buildHttpPost({ question: msg }, authService)
    );
  ...
  public async cancelResponse(sessionId: string, authService?: AuthService): Promise<void> {
    const url = `${this.sessionUrl(sessionId)}/cancel`;
    await fetchJson(url, await this.buildHttpPost({}, authService));
  }
}
