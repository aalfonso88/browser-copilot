...
export class AgentSession {
  ...
  private solveUrlTemplate(urlTemplate: string, baseUrl: string): string {
    ...
    let ret = urlTemplate
      .replace("${origin}", parsedUrl.origin)
      .replace("${basePath}", basePath)
      .replace("${sessionId}", this.id!);
    return ret;
  }
  ...
  private async updateRequestRules() {
    ...
    let requestRules = this.agent.manifest.onHttpRequest.flatMap((r) =>
      r.actions
  ...
  private buildModifyHeadersRule(
    ruleId: number,
    condition: AgentRuleCondition,
    action: AddHeaderRuleAction
  ...
  private async findInteraction(req: RequestEvent, action: RecordInteractionRuleAction): Promise<any> {
    if (action.httpRequest) {
      return await fetchJson(this.solveUrlTemplate(action.httpRequest.url, req.details.url), {
        method: action.httpRequest.method,
      });
    } else {
  ...
  public async processUserMessage(
    ...
    stepHandler?: (step: AgentFlow) => void
  ) {
    try {
      if (file.data) {
  ...
  public async close() {
    ...
    try {
      ...
      if (endAction) {
        await fetchJson(this.solveUrlTemplate(endAction.httpRequest.url, this.url), {
          method: endAction.httpRequest.method,
        });
      }
    } catch (e) {
      ...
      console.error(
        `Problem closing session. Please contact support at ${this.agent.manifest.contactEmail} with the details.`,
        e
      );
    }
  }
