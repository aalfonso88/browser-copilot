...
const onAgentActivation = (msg: AgentActivation) => {
  ...
  if (!msg.success) {
    const text = t("activationError", {
      agentName: msg.agent.manifest.name,
      contactEmail: msg.agent.manifest.contactEmail,
    });
...
const onInteractionSummary = (msg: InteractionSummary) => {
  ...
  messages.value.splice(
    messagePosition,
    0,
    msg.success ? ChatMessage.agentMessage(text) : ChatMessage.agentErrorMessage(text)
  );
};
...
const onAgentError = (error: any) => {
  ...
  if (error instanceof HttpServiceError && error.detail) {
    ...
    text = t("flowStep" + error.errorCode, {
      selector: error.step.selector,
